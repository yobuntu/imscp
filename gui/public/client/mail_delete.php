...
function client_deleteMail($mailId, $dmnProps)
{
	...
	if ($stmt->rowCount()) {
		...
		if (
			...
			//preg_match('/' . MT_NORMAL_MAIL . '/', $data['mail_type']) ||
			//preg_match('/' . MT_NORMAL_FORWARD . '/', $data['mail_type'])
		) { // Mail to dmn
			...
			//preg_match('/' . MT_ALIAS_MAIL . '/', $data['mail_type']) ||
			//preg_match('/' . MT_ALIAS_FORWARD . '/', $data['mail_type'])
		) { // mail to als
			...
			//preg_match('/' . MT_SUBDOM_MAIL . '/', $data['mail_type']) ||
			//preg_match('/' . MT_SUBDOM_FORWARD . '/', $data['mail_type'])
		) { // mail to sub
			$stmt = exec_query("SELECT `subdomain_name` FROM `subdomain` WHERE `subdomain_id` = ?", $data['sub_id']);

			if ($stmt->rowCount()) {
				$mailAddr = $data['mail_acc'] . '@' . $stmt->fields['subdomain_name'] . '.' .
					$dmnProps['domain_name'];
			} else {
				throw new iMSCP_Exception('Mail account data not found', 500);
			}
		} elseif (
			...
			//preg_match('/' . MT_ALSSUB_MAIL . '/', $data['mail_type']) ||
			//preg_match('/' . MT_ALSSUB_FORWARD . '/', $data['mail_type'])
		) { // mail to als
			$query = '
				SELECT
